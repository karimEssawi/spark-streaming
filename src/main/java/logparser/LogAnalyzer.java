...
public class LogAnalyzer {
    ...
    private static class ValueComparator<K, V> implements Comparator<Tuple2<K, V>>, Serializable {
        ...
        @Override
        public int compare(Tuple2<K, V> a, Tuple2<K, V> b) {
            return comparator.compare(a._2(), b._2());
        }
    }

    public static void main(String... args) {
        SparkConf conf = new SparkConf().setAppName("Log Analyzer").setMaster("local[2]");
        ...
        // Note how the contentSizes are cached as well since multiple actions are called on that RDD.
        JavaRDD<Long> contentSizes = accessLogs.map(ApacheAccessLog::getContentSize).cache();
        System.out.println(String.format("Content Size Avg: %s, Min: %s, Max: %s",
                contentSizes.reduce(SUM_REDUCER) / contentSizes.count(),
                contentSizes.min(Comparator.<Long>naturalOrder()),
                contentSizes.max(Comparator.<Long>naturalOrder())));

        // Compute response code count
        List<Tuple2<Integer, Long>> responseCodeCount = accessLogs.mapToPair(log -> new Tuple2<>(log.getResponseCode(), 1L))
                                                              .reduceByKey(SUM_REDUCER)
                                                              .take(100);
        System.out.println(String.format("Response code count: %s", responseCodeCount));
        ...
        List<String> ipAddresses = accessLogs.mapToPair(log -> new Tuple2<>(log.getIpAddress(), 1L))
                                             ...
                                             .map(Tuple2::_1)
                                             .take(100);
        System.out.println(String.format("IP Addresses > 10 times: %s", ipAddresses));
